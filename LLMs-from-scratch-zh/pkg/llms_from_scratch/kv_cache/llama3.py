...
class Llama3Tokenizer:
    """对 tiktoken 的轻量封装，用于跟踪 Llama-3 的特殊 ID。"""
...
class GroupedQueryAttentionFast(nn.Module):
    """
    GroupedQueryAttention 的即插即用替代实现，但使用 PyTorch 的
    scaled_dot_product_attention；在 Ampere GPU（如 A100）或更新设备上、
    且使用 float16/bfloat16 或更低精度时，会调用 FlashAttention。
    """
...
class TransformerBlockFast(nn.Module):
    """
    与原始 TransformerBlock 相同，但使用 GroupedQueryAttentionFast
    替代 GroupedQueryAttention。
    """
    def __init__(self, cfg):
        super().__init__()
...
class Llama3ModelFast(nn.Module):
    """
    与原始 Llama3Model 相同，但使用 TransformerBlockFast
    替代 TransformerBlock，而 TransformerBlockFast 内部
    使用 GroupedQueryAttentionFast 替代 GroupedQueryAttention。
    """
    def __init__(self, cfg):
        super().__init__()
