...
class Llama3Tokenizer:
    """围绕 tiktoken 的轻量级包装,用于跟踪 Llama-3 的特殊 ID。"""
...
class GroupedQueryAttentionFast(nn.Module):
    """
    GroupedQueryAttention 的直接替换实现,但使用 PyTorch 的
    scaled_dot_product_attention,后者在 Ampere GPU(如 A100)或更新架构
    上并使用 float16/bfloat16 或更低精度时会启用 FlashAttention。
    """
...
class TransformerBlockFast(nn.Module):
    """
    与原始 TransformerBlock 相同,但使用
    GroupedQueryAttentionFast 来代替 GroupedQueryAttention。
    """
    def __init__(self, cfg):
        super().__init__()
...
class Llama3ModelFast(nn.Module):
    ...
    来代替 TransformerBlock,而 TransformerBlockFast 又使用
    GroupedQueryAttentionFast 来代替 GroupedQueryAttention。
    """
    def __init__(self, cfg):
        super().__init__()
