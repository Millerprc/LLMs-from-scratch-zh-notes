...
这份补充材料说明了如何在 GPT 模型训练设置中使用 PyTorch 的 Muon 优化器。

&nbsp;
## 引言

Muon（["Muon is Scalable for LLM Training"](https://arxiv.org/abs/2502.16982)）是一个相对较新的优化器，用于训练 LLM 的大型 2D 权重矩阵（这些矩阵在 transformer block 中占主导地位），例如注意力投影、前馈投影和输出 head。但是，不适合 Muon 的参数（例如 embeddings、biases 和 normalization 参数）通常仍保留在 AdamW 上。

具体来说，这意味着：

1. 对不属于 embedding 层的可训练 2D 参数使用 Muon。
2. 对 embeddings、biases、normalization 参数以及其他非 2D 参数使用 AdamW。
3. 在学习率方面，例如，尝试对 Muon 使用 `lr=1e-4`，对 AdamW 使用 `lr=5e-5`，`weight_decay=0.1`，并对 Muon 使用 `adjust_lr_fn="match_rms_adamw"`。

&nbsp;
## 代码示例

[gpt_train.py](gpt_train.py) 脚本是第 5 章的基线脚本：
...
替代的 [gpt_train_muon.py](gpt_train_muon.py) 脚本从相同的模型实现开始，但使用 Muon（除了 AdamW 之外）。
...
顺便说一下，这不是作为有意义的语言建模基准。模型是随机初始化的，仅在一个微小的重复文本片段上训练一个 epoch，以便优化器路径易于检查且快速运行。